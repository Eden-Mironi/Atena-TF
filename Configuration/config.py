...
humanity_coeff = 1.0  # REVERTED: Experiment showed 0.5 was worse - keep full Snorkel signal

# Snorkel-based Reward
use_snorkel = True  # Enable Snorkel classification system
snorkel_warmup_steps = 0  # Set to 0 to enable Snorkel immediately (was 500000 for initial training)
                          # When resuming from step 500K+, we want Snorkel active from the start
                          # Original: 500000 (disable for first 500K steps during initial training)
...
ppo_update_interval = 1024  # Update interval (reduced from 2048 to prevent OOM)
batchsize = 64  # Minibatch size
epochs = 10  # Training epochs per update
entropy_coef = 0.1  # MASTER EXACT: No entropy regularization (master's default)

# Reward Scaling (CRITICAL!)
...
REWARD_COEFFICIENTS_MASTER_EXACT = {
    'kl_coeff': 1.5,           # MASTER EXACT: From master's arguments.py line 102
    'compaction_coeff': 2.0,   # MASTER EXACT: From master's arguments.py line 103
    'diversity_coeff': 2.0,    # MASTER EXACT: From master's arguments.py line 101
    'humanity_coeff': 1.0,     # MASTER EXACT: From master's arguments.py line 100
}
...
if USE_MASTER_EXACT_COEFFICIENTS:
    ...
    entropy_coef = 0.0  # MASTER EXACT: Master uses entropy_coef=0.0 (arguments.py line 139)
