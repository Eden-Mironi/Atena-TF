...
def tokenize_column(df, column):
    ...
    for idx, line in df[column].items():
        tokens = tokenize_line_of_column(line, column)
        add_tokens_to_dict(tokens, tokens_dict, len_df)

    # FIX: Filter out nan and <UNK> from available tokens
    # These are placeholder values that shouldn't be selectable as filter terms
    invalid_tokens = {nan, '<UNK>', 'nan', float('nan')}
    tokens_dict = {k: v for k, v in tokens_dict.items() 
                   if not (k in invalid_tokens or (not isinstance(k, str) and math.isnan(k)))}
    
    # If no valid tokens remain, return empty lists (will trigger <UNK> fallback)
    if not tokens_dict:
        return [], []
