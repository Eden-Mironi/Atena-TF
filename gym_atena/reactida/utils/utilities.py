...
class Repository:
    ...
    def _preprocess_dataframe(df):
        """
        HYBRID SOLUTION: Preprocess dataframe to handle missing values
        
        This implements ATENA-master's preprocessing approach while keeping our
        generic penalty system as backup protection.
        
        Strategy:
        1. Fill ALL numeric columns with -1
        2. Fill ALL string/object columns with appropriate placeholders
        3. Keep penalty system (-20.0) as safety net for edge cases
        
        Args:
            df: Raw dataframe from TSV file
            
        Returns:
            Preprocessed dataframe with NaN values filled
        """
        # Make a copy to avoid pandas SettingWithCopyWarning
        df = df.copy()
        
        # Process EVERY column generically based on dtype
        for col in df.columns:
            if df[col].isna().any():  # Only process columns with NaN
                # Check if column is numeric
                if pd.api.types.is_numeric_dtype(df[col]):
                    # Numeric columns: Fill with -1
                    df[col] = df[col].fillna(-1).astype('int64')
                else:
                    # String/object columns: Fill based on column type
                    if 'ip' in col.lower():
                        df[col] = df[col].fillna('0.0.0.0').astype(str)
                    elif 'mac' in col.lower() or 'eth' in col.lower():
                        df[col] = df[col].fillna('00:00:00:00:00:00').astype(str)
                    else:
                        # Generic string columns: Fill with 'unknown'
    ...
    def __init__(self, actions_tsv, display_tsv,raw_datasets, schema=KEYS):
        ...
        for f in self.file_list:
            ...
            # HYBRID FIX: Preprocess data to fill NaN values (like ATENA-master)
            # This reduces <UNK> occurrences during training for known problematic columns
            # Combined with penalty system (-20.0) for complete protection
            nan_before = df.isna().sum().sum()
            df = self._preprocess_dataframe(df)
            nan_after = df.isna().sum().sum()
            # Suppress output for faster notebook loading
            # print(f"PREPROCESSING: {f} - NaN {nan_before} → {nan_after}")
