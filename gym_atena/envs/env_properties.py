...
class EnvDatasetProp(object):
    ...
    def __init__(self,
                 ...
                 NUMERIC_KEYS,
                 rule_based_humanity_score_func,
                 FILTER_LIST=None,
                 FILTER_BY_FIELD_DICT=None,
                 DONT_FILTER_FIELDS=None,
                 snorkel_data_obj=None,
                 snorkel_gen_model=None,
                 snorkel_training_path=None,
                 ):
        """
        ...
        Args:
            ...
            rule_based_humanity_score_func(Callable[..., float]): A function that returns a coherency score for the
        ...
        self.snorkel_data_obj = snorkel_data_obj
        self.snorkel_gen_model = snorkel_gen_model
        self.snorkel_training_path = snorkel_training_path
        self.rule_based_humanity_score_func = rule_based_humanity_score_func

    def get_snorkel_gen_model(self):
        """CRITICAL FIX: Add missing get_snorkel_gen_model() method (matching master exactly)"""
        # Preventing from snorkel to print to console
        # old_stdout = sys.stdout
        # sys.stdout = open(os.devnull, "w")
        snorkel_gen_model = None
        if cfg.use_snorkel:
            snorkel_gen_model = self.snorkel_gen_model()
        # sys.stdout = old_stdout
        return snorkel_gen_model
...
class EnvProp(object):
    ...
    def get_filtered_df(self, dataset_df, filter_tpl):
        ...
        for filt in filter_tpl:
            ...
            if opr is not None:
                try:
                    # FIX: Handle <UNK> and invalid values gracefully to prevent TypeError
                    if value == '<UNK>':
                        # <UNK> means no valid filter value - return empty dataframe
                        logger.warning(f"Filter on column {field} with <UNK> value - returning empty result")
                        return df.truncate(after=-1)
                    
                    # Convert value to match column dtype
                    col_dtype = str(df[field].dtype)
                    if col_dtype not in ['object', 'category']:
                        # Numeric column - convert value to float
                        try:
                            value = float(value)
                        except (ValueError, TypeError):
                            # Can't convert to float - invalid value for numeric column
                            logger.warning(f"Filter on numeric column {field} with non-numeric value '{value}' - returning empty result")
                            return df.truncate(after=-1)
                    ...
                    logger.warning(f"Filter on column {field} with operator {opr} and value {value} (type: {type(value).__name__}) failed: {e}")
                    return df.truncate(after=-1)
            else:
                # print("***"+field+"\n\n" + str(op_num) +"\n\n" + str(opr)+"\n\n***")
                """
